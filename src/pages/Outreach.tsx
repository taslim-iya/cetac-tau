...
export default function Outreach() {
  const { outreach, update, add, remove, removeMany, updateMany, reorder } = useStore();
    ...
    <div style={{ padding: '32px 40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-2)', marginTop: 2 }}>{outreach.length} messages · drag to reorder</p>
        </div>
        <button className="btn btn-primary" onClick={() => add('outreach', { contactName: '', contactEmail: '', subject: '', message: '', status: 'draft', sentDate: '', followUpDate: '', category: '', notes: '' })}>
          + Add Outreach
        </button>
      </div>
      <DataTable
        ...
        onReorder={(ids) => reorder('outreach', ids)}
        onBulkDelete={(ids) => removeMany('outreach', ids)}
        onBulkUpdate={(ids, updates) => updateMany('outreach', ids, updates)}
        entityName="outreach"
      />
    </div>
  );
